...
class test extends Controller
{
    ...
    public function order_userchange()
    {
        ...
        foreach ($addresses as $key => $value) {
            $addressArray[] = ['id' => $value->id, 'value' => htmlentities($value->printFullAddressWithPhoneEmail())];
        }

        return ['addresses' => $addressArray, 'default_address' => $default_address->id, 'min_amount' => $user->minimum_amount()];
    }
        ...
        if ($pp->product->product_type == COMBINED_PRODUCT) {
            ...
            foreach ($product->ProductCombination() as $combination) {
                ...
                if (isset($attr[$cpid][0])) {
                    $arr = [$cpid => [$cpvid => ['qty' => $attr[$cpid][0]],
                    ]];
                } else {
                    $arr = [$cpid => [$cpvid => ['qty' => 1],
                    ]];
                }
                $cbattributes = [];
            ...
            if ($variations) {
                foreach ($variations as $key => $value) {
                    if (isset($input['attribute'][$value['variation_id']])) {
                        $attributes[] = [$value['name'] => $input['attribute'][$value['variation_id']]];
                    } else {
        ...
        if ($variations) {
            foreach ($variations as $key => $value) {
                if (isset($attr[$cpid][$value['variation_id']])) {
                    $cbattributes[] = [$value['name'] => $attr[$cpid][$value['variation_id']]];
                } else {
                    $cbattributes[] = [
    ...
    private function GetCustomProductAttributes($pp, $combination, $cpid, $cpvid, $attr)
    {
        ...
        $cbattributes = [];
        $variations = $ppv->productVariations();
        if ($variations) {   //printe($variations,$attr[$cpid][$combination_variation_id]);
            foreach ($variations as $key => $value) {
                if (isset($attr[$cpid][$combination_variation_id][$value['variation_id']])) {
                    $cbattributes[] = [$value['name'] => $attr[$cpid][$combination_variation_id][$value['variation_id']]];
                } else {
                    $cbattributes[] = [
                        $value['name'] => $value['value'], ];
                }
